...
function Playlist(title, author, dateCreated, songs) {
    ...
    this.play = function (songId) {
        const song = this.songs.find(element => element.id === songId)
...
Playlist.prototype.addToPlaylist = function (id, title, artist, year) {
    const newSong = { id, title, artist, year }
...
const andrewPlaylist = new Playlist('Cool tunes', 'Andrew', new Date(), songs)

// console.log(andrewPlaylist)

// andrewPlaylist.play(63)
// andrewPlaylist.play(634)

andrewPlaylist.addToPlaylist(500, 'Cotton eyed joe', 'Maraiah Carrey', 2018);